...
"""Implementation of the path distance node feature extractor."""
...
class ExtractPathDistances(ExtractDistances):
    ...
    For each apical tree in the neuron the path distance of all node points
    of the morphology are extracted and added to the feature vector.

    The path distance of a given node is the distance to the root point of the
    tree that is measured by following the graph edges.
    ...
    def get_distances(self, apical):
        ...
        """
        path_distances = apical.get_point_path_distances()
        path_distances = torch.tensor(path_distances, dtype=torch.float32)
